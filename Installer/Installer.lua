...
if component.isAvailable("tablet") then table.insert(govno, "Tablet PC detected - You can't install MineOS_Server on tablet because of primitive GPU and Screen.") end

--Проверяем GPU
if gpu.maxResolution() < 150 then table.insert(govno, "Bad GPU or Screen - MineOS_Server requires Tier 3 GPU and Tier 3 Screen.") end

--Проверяем оперативку
if math.floor(computer.totalMemory() / 1536 ) < 2000 then table.insert(govno, "Not enough RAM - MineOS_Server Server requires at least 2000 KB RAM.") end

if fs.get("bin/edit.lua") == nil or fs.get("bin/edit.lua").isReadOnly() then table.insert(govno, "You can't install MineOS_Server Server on floppy disk. Run \"install\" in command line and install OpenOS from floppy to HDD first. After that you're be able to install MineOS_Server Server from Pastebin.") end
...
local preLoadApi = {
  { paste = "FrisKAY/MineOS_Server/master/lib/ECSAPI.lua", path = "lib/ECSAPI.lua" },
  { paste = "FrisKAY/MineOS_Server/master/lib/colorlib.lua", path = "lib/colorlib.lua" },
  { paste = "FrisKAY/MineOS_Server/master/lib/image.lua", path = "lib/image.lua" },
  { paste = "FrisKAY/MineOS_Server/master/lib/config.lua", path = "lib/config.lua" },
  { paste = "FrisKAY/MineOS_Server/master/MineOS_Server/Icons/Languages.pic", path = "MineOS_Server/System/OS/Icons/Languages.pic" },
  { paste = "FrisKAY/MineOS_Server/master/MineOS_Server/Icons/OK.pic", path = "MineOS_Server/System/OS/Icons/OK.pic" },
  { paste = "FrisKAY/MineOS_Server/master/MineOS_Server/Icons/Downloading.pic", path = "MineOS_Server/System/OS/Icons/Downloading.pic" },
  { paste = "FrisKAY/MineOS_Server/master/MineOS_Server/Icons/OS_Logo.pic", path = "MineOS_Server/System/OS/Icons/OS_Logo.pic" },
}

print("Downloading file list")
applications = seri.unserialize(getFromGitHubSafely(GitHubUserUrl .. "FrisKAY/MineOS_Server/master/Applications.txt", "MineOS_Server/System/OS/Applications.txt"))
print(" ")
...
local imageOS = image.load("MineOS_Server/System/OS/Icons/OS_Logo.pic")
local imageLanguages = image.load("MineOS_Server/System/OS/Icons/Languages.pic")
local imageDownloading = image.load("MineOS_Server/System/OS/Icons/Downloading.pic")
local imageOK = image.load("MineOS_Server/System/OS/Icons/OK.pic")
...
do
  ...
  local pathToLang = "MineOS_Server/System/OS/Installer/Language.lang"
  getFromGitHubSafely(GitHubUserUrl .. "FrisKAY/MineOS_Server/master/Installer/" .. _G.OSSettings.language .. ".lang", pathToLang)
  getFromGitHubSafely(GitHubUserUrl .. "FrisKAY/MineOS_Server/master/MineOS_Server/License/" .. _G.OSSettings.language .. ".txt", "MineOS_Server/System/OS/License.txt")
...
do
  ...
  local lines = {}
  local file = io.open("MineOS_Server/System/OS/License.txt", "r")
...
local desktopPath = "MineOS_Server/Desktop/"
local dockPath = "MineOS_Server/System/OS/Dock/"
local applicationsPath = "MineOS_Server/Applications/"
local picturesPath = "MineOS_Server/Pictures/"
...
if downloadWallpapers then ecs.createShortCut("MineOS_Server/System/OS/Wallpaper.lnk", picturesPath .. "Girl.pic") end
