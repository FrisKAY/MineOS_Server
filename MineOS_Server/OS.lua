...
local copyright = [[
    ...
    Изначально эту ОСь накодил Тимофеев Игорь,
	ссылка на ВК: vk.com/id7799889
	Но сделал адаптацию под сервера Александр Максименко,
	ссылка на ВК: vk.com/id_friskay1999
...
local lang = config.readAll("MineOS_Server/System/OS/Languages/" .. _G.OSSettings.language .. ".lang")

---------------------------------------------- Переменные ------------------------------------------------------------------------

local workPath = "MineOS_Server/Desktop/"
local pathOfDockShortcuts = "MineOS_Server/System/OS/Dock/"
local pathToWallpaper = "MineOS_Server/System/OS/Wallpaper.lnk"
...
local function drawTopBar()
	--Элементы топбара
	local topBarElements = { "Сервер", "Вид" }
...
local function login()
	...
	if not _G.OSSettings.protectionMethod then
		while true do
			local data = ecs.universalWindow("auto", "auto", 30, ecs.windowColors.background, true, {"EmptyLine"}, {"CenterText", 0x000000, "Защитите сервер если это надо!"}, {"EmptyLine"}, {"Input", 0x262626, 0x880000, "Пароль"}, {"Input", 0x262626, 0x880000, "Подтвердить пароль"}, {"EmptyLine"}, {"Button", {0xAAAAAA, 0xffffff, "OK"}, {0x888888, 0xffffff, "Публичный сервер"}})
			if data[3] == "OK" then
		...
		while true do
			...
			elseif hash == "29f4549f93d5bdae123bc1a0d03127291d16d15bc8260be21199a2c2443f825e" then
				ecs.universalWindow("auto", "auto", 30, ecs.windowColors.background, true,
					{"EmptyLine"},
					{"CenterText", 0x880000, "MineOS_Server"}, 
					{"EmptyLine"},
...
local function windows10()
	...
	local function draw(background)
		...
		buffer.text(x + 2, y + 1, 0xFFFFFF, "Получите MineOS Server R2")
		buffer.text(x + width - 3, y + 1, 0xFFFFFF, "X")

		buffer.image(x + 2, y + 4, image.load("MineOS_Server/System/OS/Icons/Computer.pic"))

		buffer.text(x + 12, y + 4, 0xFFFFFF, "Ваш MineOS Server R1 готов для")
		buffer.text(x + 12, y + 5, 0xFFFFFF, "бесплатного обновления")

		buffer.text(x + 2, y + height - 2, 0x999999, "Через некоторое время вы")
		buffer.text(x + 2, y + height - 1, 0x999999, "сможете бесплатно установить")
		buffer.text(x + 20, y + height - 1, background, "MineOS Server R2")
	...
	while true do
		...
		elseif ecs.clickedAtArea(eventData[3], eventData[4], x, y, x + width - 1, x + height - 1) then
			...
			local data = ecs.universalWindow("auto", "auto", 30, ecs.windowColors.background, true, {"EmptyLine"}, {"CenterText", 0x000000, "  На данный момент, MineOS Server R2 только тестируеться!  "}, {"CenterText", 0x000000, "  Мы сообщим вам когда будет обновление!  "}, {"EmptyLine"}, {"Button", {0xbbbbbb, 0xFFFFFF, "Ок"}, {0x999999, 0xFFFFFF, "Отключить оповещения"}})
			if data[1] == "Да" then
				disableUpdates()
			else
				ecs.error("Оповещение отключено")
			end
			...
			if ecs.clickedAtArea(eventData[3], eventData[4], obj["DesktopIcons"][key][1], obj["DesktopIcons"][key][2], obj["DesktopIcons"][key][3], obj["DesktopIcons"][key][4]) then
				...
				if eventData[5] == 0 then
					...
					if fs.isDirectory(path)	then
						if fileFormat == ".app" then
							...
							shell.execute("MineOS_Server/Applications/Finder.app/Finder.lua " .. path)
							drawAll()
						end
					else
						ecs.launchIcon(path)
				...
				elseif eventData[5] == 1 then
					...
					elseif action == lang.contextAddToDock then
						ecs.createShortCut("MineOS_Server/System/OS/Dock/" .. ecs.hideFileFormat(path) .. ".lnk", path)
						drawAll()
					...
					elseif action == "Редактировать в Photoshop" then
						shell.execute("MineOS_Server/Applications/Photoshop.app/Photoshop.lua open " .. path)
						drawAll(true)
					else
