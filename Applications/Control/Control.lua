...
local osIcon = image.load("MineOS_Server/System/OS/Icons/OS_Logo.pic")
local hddIcon = image.load("MineOS_Server/System/OS/Icons/HDD.pic")
local floppyIcon = image.load("MineOS_Server/System/OS/Icons/Floppy.pic")
local updateIcon = image.load("MineOS_Server/System/OS/Icons/Update.pic")
...
local function drawMain()
	...
	if currentMode == 1 then
		...
		ecs.colorText(xPos, yPos, ecs.colors.lightGray, "Публичная бета-версия 1.1.5"); yPos = yPos + 2

		ecs.smartText(xPos, yPos, "§fСистемный блок §8(Серверный блок)"); yPos = yPos + 1
		ecs.smartText(xPos, yPos, "§fПроцессор §8(Серверный процессор 3-го уровня, 4.8 GHz)"); yPos = yPos + 1
		ecs.smartText(xPos, yPos, "§fПамять §8(1333 МГц DDR3 "..ram.total.." KB)"); yPos = yPos + 1
		ecs.smartText(xPos, yPos, "§fГрафика §8(GTX Titan X 12GB DDR4)"); yPos = yPos + 1
	...
	elseif currentMode == 2 then
		...
		for i = drawHDDFrom, (drawHDDFrom + 3) do
			...
			local load = ""
			if bootAddress == HDDs[i].address then load = " §eзагрузочный диск (Основной)§8," end
			...
			yPos = yPos + 1
			ecs.colorTextWithBack(xPos + 10, yPos, 0xaaaaaa, colors.main, HDDs[i].spaceUsed.." из "..HDDs[i].spaceTotal.." KB занято"); yPos = yPos + 1
		...
		if currentMode == 2 then
			for key in pairs(obj["HDDControls"]) do
				if ecs.clickedAtArea(e[3], e[4], obj["HDDControls"][key][1], obj["HDDControls"][key][2], obj["HDDControls"][key][3], obj["HDDControls"][key][4]) then
					...
					local action = context.menu(obj["HDDControls"][key][1], obj["HDDControls"][key][2] + 3, {"Форматировать"}, {"Изменить имя"}, {"Включить диск как основной"}, "-", {"Сдублировать OS на этот диск"})
