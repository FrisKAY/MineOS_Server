...
local leftBar
local pathToConfig = "MineOS_Server/System/Finder/Config.cfg"
...
local function loadConfig()
	if fs.exists(pathToConfig) then
		...
		leftBar = {
			{"Title", "Избранное"},
			{"Element", "Root", ""},
			{"Element", "System", "MineOS_Server/System/"},
			{"Element", "Libraries", "lib/"},
			{"Element", "Scripts", "bin/"},
			{"Element", "Desktop", "MineOS_Server/Desktop/"},
			{"Element", "Applications", "MineOS_Server/Applications/"},
			{"Element", "Pictures", "MineOS_Server/Pictures/"},
			{"Title", "", ""},
			{"Title", "Диски"},
		}
...
local args = { ... }
-- local cykaImage = image.load("MineOS_Server/Pictures/AhsokaTano.pic")
			...
			if ecs.clickedAtArea(e[3], e[4], obj["Icons"][key][1], obj["Icons"][key][2], obj["Icons"][key][3], obj["Icons"][key][4]) then
				...
				--А если правая
					...
					elseif action == "Загрузить на Pastebin" then
						shell.execute("MineOS_Server/Applications/Pastebin.app/Pastebin.lua upload " .. path)
						getFileList(workPathHistory[currentWorkPathHistoryElement])
						drawAll(true)
					elseif action == "Установить как обои" then
						--ecs.error(path)
						ecs.createShortCut("MineOS_Server/System/OS/Wallpaper.lnk", path)
